...
import '../common/default_grid_sampler.dart';
...
import '../qrcode/decode_hint.dart';
...
class Detector {
  ...
  DetectorResult detect([DecodeHint? hints]) {
    final finder = FinderPatternFinder(_image);
  ...
  int _computeDimension(
    ...
    try {
      if (dimension >= 43) {
        final versionBits =
            _readTopRightVersionBits(topLeft, topRight, bottomLeft, dimension);
        final version1 = Version.decodeVersionInformation(versionBits);
        //final version2 = Version.decodeVersionInformation(versionBits.$2);

        print('my: ${version1?.dimensionForVersion}');
        //print('my: ${version2?.dimensionForVersion}');

        if (version1 != null) dimension = version1.dimensionForVersion;
      }
    } catch (e) {
      // If reading version information fails, fall back to the computed dimension
      print(e);
    }
    print('');
  ...
  int _readTopRightVersionBits(
      ResultPoint tl, ResultPoint tr, ResultPoint bl, int dimension) {
    int startX = dimension - 11;
    int startY = 0;

    final transform = _createTransform(tl, tr, bl, null, dimension);
    ...
    for (int j = 5; j >= 0; j--) {
      for (int i = 2; i >= 0; i--) {
        versionBits = (versionBits << 1) | (versionMatrix.get(i, j) ? 1 : 0);
      }
    }
